...
if uploaded_file is not None:
    ...
    if df.empty:
        st.error("⚠️ Dataset kosong atau format tidak sesuai. Pastikan file berisi riwayat trading yang valid.")
    else:
        def clean_money(x):
        ...
        koin_tersering = most_traded.index[0] if not most_traded.empty else "N/A"
        col7.metric("🎯 Koin Tering", f"{koin_tersering}")
        ...
        with col_v1:
            ...
            jam_terburuk = jam_stats['Loss'].idxmax() if not jam_stats[jam_stats['Loss'] > 0].empty else "N/A"
